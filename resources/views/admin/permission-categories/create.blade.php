<x-layouts.dashboard>
    <div class="max-w mx-auto p-4">
        ...
        <div class="flex justify-between items-center mb-4">
            <h1 class="text-2xl font-semibold">Create Permission Category</h1>
            <a href="{{ route('permission-categories.index') }}" class="btn btn-outline btn-neutral">Back to Permission Categories</a>
        </div>

        <div class="bg-white border border-gray-200 rounded-lg shadow-sm">
            <div class="p-6">
                <h2 class="text-xl font-semibold text-primary mb-6">Permission Category Information</h2>

                <form method="POST" action="{{ route('permission-categories.store') }}">
                    @csrf
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="3"
                                      class="textarea focus:border-none textarea-primary w-full @error('description') textarea-error @enderror">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="text-error text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="flex gap-4 mt-6">
                        <button type="submit" class="btn btn-primary btn-outline">Create Category</button>
                        <a href="{{ route('permission-categories.index') }}" class="btn btn-neutral btn-outline">Cancel</a>
                    </div>
                </form>
            </div>
        </div>
